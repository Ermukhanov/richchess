...
function OnlineGame() {
  ...
  useEffect(() => {
    ...
    if (!savedRef.current && user && color !== "spectator") {
      savedRef.current = true;
      const eloDelta = result === "win" ? 20 : result === "loss" ? -15 : 0;
      const opp = players.find((p) => p.user_id !== user.id && p.color !== "spectator");
  ...
  const resign = () => {
    if (!color || color === "spectator" || over) return;
  ...
  const myTurn = color !== "spectator" && color === chess.turn();
  const orientation = color === "b" ? "black" : "white";
  const opp = players.find((p) => p.user_id !== user?.id && p.color !== "spectator");
  const waiting = color !== "spectator" && players.filter((p) => p.color !== "spectator").length < 2;
        ...
        <div className="grid lg:grid-cols-[1fr,320px] gap-6">
          <div className="relative">
            <div className="bg-card border border-border rounded-2xl p-3 md:p-4">
              <div className="flex items-center justify-between text-xs uppercase tracking-wider text-muted-foreground mb-2 px-1">
                ...
                <span>{color === "spectator" ? "Spectator mode" : myTurn && !over ? t("yourTurn") : ""}</span>
              </div>
              <ChessGame
                externalChess={chess}
                fen={fen}
                disabled={!myTurn || !!over || waiting || color === "spectator"}
          ...
          <aside className="bg-card border border-border rounded-2xl p-4 flex flex-col gap-4">
            <ChessClock
              ...
              onFlag={(c) => {
                if (over || color === "spectator") return;
